...
const PatientReg = () => {
  ...
  function onSubmitClick(event) {
    ...
    const patientReg = {
      ...
      dob: dob,
      is_Patient: true,
      patientDetailDTO: { patientName: firstname, city: city, address: address, addedDate: null, stateId: 1 },
      doctorDTO: null
    };
    console.log(JSON.stringify(patientReg));

    // axios({
    //   method: 'post',
    //   url: 'http://localhost:9000/registration/register/',
    //   data: JSON.stringify(patientReg)
    // })
    ...
    fetch('http://localhost:9000/registration/register', {
      method: 'POST',
      crossDomain: true,
      body: JSON.stringify(patientReg),
      headers: {
